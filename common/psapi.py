...
def get_podcast_episodes(podcast_id, season = None, format = "json", page = 1):
    logging.info(f"Fetching episodes for podcast {podcast_id} ({season})...")

    url = f"{api_base_url}/radio/catalog/podcast/{podcast_id}/episodes?page={page}&pageSize={page_size}&sort=desc"
    if season:
